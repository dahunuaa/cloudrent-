dhtmlXGridObject.prototype.enablePaging = function(fl, pageSize, pagesInGrp,
	...
	this._pgn_recInfoParentObj = typeof(recInfoParentObj) == "string"
			? document.getElementById(recInfoParentObj)
			: recInfoParentObj;
	this.pagingOn = fl;
...
dhtmlXGridObject.prototype._pgn_block = function(sep) {
	...
	for (var i = start + 1; i <= max; i++)
		str.push("<a href='#' onclick='this.grid.changePage(" + i + ")'>"
				+ (i == this.currentPage ? ("<b>" + i + "</b>") : i) + "<a>");
	return str.join(sep)
};
dhtmlXGridObject.prototype._pgn_link = function(mode, ac, ds) {
	if (mode == "prevpages" || mode == "prev") {
		if (this.currentPage == 1)
			return ds;
		return '<a href=\'#\' onclick=\'this.grid.changePageRelative(-1*this.grid.pagesInGroup)\'>'
				+ ac + '</a>'
	};
	if (mode == "nextpages" || mode == "next") {
		if (this.rowsBuffer.length / this.rowsBufferOutSize <= this.currentPage)
			return ds;
		return '<a href=\'#\' onclick=\'this.grid.changePageRelative(this.grid.pagesInGroup)\'>'
				+ ac + '</a>'
	};
	if (mode == "current") {
		...
		return '<a href=\'#\' '
				...
				+ 'onclick=\'this.grid.changePage(' + i + ')\'>' + i + '</a>'
	};
	return ac
};
...
dhtmlXGridObject.prototype._pgn_toolbar = function(page, start, end) {
	if (!this.aToolBar)
		this.aToolBar = this._pgn_createAToolBar();
	var totalPages = Math.ceil(this.rowsBuffer.length / this.rowsBufferOutSize);
	if (this._WTDef[2]) {
	...
	if (this._WTDef[1]) {
		var iButton = this.aToolBar.getItem("results");
		if (!this.getRowsNum())
			iButton.setText(this.i18n.paging.notfound);
		else
			iButton.setText(this.i18n.paging.records + (start + 1)
					+ this.i18n.paging.to + end)
	};
	if (this._WTDef[3])
		this.aToolBar.getItem("perpagenum").setSelected(this.rowsBufferOutSize
				.toString())
...
dhtmlXGridObject.prototype._pgn_createAToolBar = function() {
	...
	this.aToolBar = new dhtmlXToolbarObject(this._pgn_parentObj, '100%', 22,
			"Grid Output");
	var self = this;
	var f1 = function(val) {
		switch (this.id) {
	...
	if (this._WTDef[0]) {
		this.aToolBar.addItem(new dhtmlXImageButtonObject(this.imgURL
						+ 'ar_left_abs.gif', "18px", "18px", f1, 'leftabs',
				this.i18n.paging.first))
		this.aToolBar.addItem(new dhtmlXImageButtonObject(this.imgURL
						+ 'ar_left.gif', "18px", "18px", f1, 'left',
				this.i18n.paging.previous))
	};
	if (this._WTDef[1])
		this.aToolBar.addItem(new dhtmlXImageTextButtonObject(this.imgURL
						+ 'results.gif', this.i18n.paging.results, 150, 18, 0,
				'results', this.i18n.paging.results))
	if (this._WTDef[0]) {
		this.aToolBar.addItem(new dhtmlXImageButtonObject(this.imgURL
						+ 'ar_right.gif', "18px", "18px", f1, 'right',
				this.i18n.paging.next))
		this.aToolBar.addItem(new dhtmlXImageButtonObject(this.imgURL
						+ 'ar_right_abs.gif', "18px", "18px", f1, 'rightabs',
				this.i18n.paging.last))
	};
	if (this._WTDef[2])
		this.aToolBar.addItem(new dhtmlXSelectButtonObject("pages", " ", " ",
				f1, "120px", "18px", "toolbar_select"));
	if (this._WTDef[3])
		this.aToolBar.addItem(new dhtmlXSelectButtonObject("perpagenum",
				"5,10,15,20,25,30", "5" + this.i18n.paging.perpage + ",10"
						+ this.i18n.paging.perpage + ",15"
						+ this.i18n.paging.perpage + ",20"
						+ this.i18n.paging.perpage + ",25"
						+ this.i18n.paging.perpage + ",30"
						+ this.i18n.paging.perpage + "", f1, "120px", "18px",
				"toolbar_select"));
...
dhtmlXGridObject.prototype._pgn_bricks = function(page, start, end) {
	...
	if (this.rowsBuffer.length > 0) {
		...
		for (var i = 1; i <= this.pagesInGroup; i++) {
			...
			lineWidth += (parseInt(lineWidthInc / 3) * pageNumber.toString().length)
					+ 15;
		...
		if (Math.ceil(page / this.pagesInGroup) * this.pagesInGroup < Math
			...
			this.pagerElAr["pagerBox"].appendChild(pageCont);
			var self = this;
			pageCont.pgnum = (Math.ceil(page / this.pagesInGroup) * this.pagesInGroup)
					+ 1;
			pageCont.onclick = function() {
				self.changePage(this.pgnum)
			};
	...
	if (this.rowsBuffer.length > 0 && this.showRecInfo == true)
