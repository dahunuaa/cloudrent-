        ...
        $http.get(basePath+get_url+"access_token="+localStorage.getItem("token")+
            ...
            .success(function(res){
                if(res.response.success == 1){
                    ...
                    for(var i=0; i < $scope.location_data.length;i++){
                        str = $scope.location_data[i]._id;
                        $scope.location_data[i].index = i+1;
                        myGrid.addRow(str,[
                            0,
                            $scope.location_data[i].index,
                            $scope.location_data[i].login_name,
                            "<button id='see_detail' style='color:#39c;cursor: pointer;'>查看详情</button>",
                            $scope.location_data[i].status.desc,
                            $scope.location_data[i].checker,
        ...
        $http.get(basePath+get_url+"access_token="+localStorage.getItem("token")+
            ...
            .success(function(res){
                if(res.response.success == 1){
                    ...
                    //添加数据
                    var get_data = $scope.location_data;
                    var str;
...
//点击事件-审核
    $("table").on('click','#check',function(){
        window.location.href = "check_page.html?"+$scope.this_row_id+"&insurance"
    });
