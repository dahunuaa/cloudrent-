...
app.controller('myCtrl', function($scope,$http) {
    ...
    function init(){
        if(localStorage.getItem("token") == undefined ||localStorage.getItem("token") == null||localStorage.getItem("token") == ""){
            ...
            myGrid.setHeader("选择,订单编号,商品名称,姓名,联系电话,支付状态,订单状态,操作");//设置表头
            myGrid.attachHeader("#master_checkbox," +
                ...
                "," +
                "," );
            myGrid.setInitWidths("60,130,100,100,100,100,100,*");//设置表格初始宽度
                ...
                if(res.response.success == 1){
                    ...
                    for(var i=0; i < get_data.length;i++){
                        ...
                        myGrid.addRow(str,[
                            0,
                            get_data[i].order_sn,
                            get_data[i].order_goods[0].goods_name,
...
//点击事件-详情
    $("table").on('click','#see_detail',function(){
        window.location.href = "hdw_order_detail.html?"+$scope.this_row_id
    });
...
//修改- 确认
            ...
            $http({
                ...
                if(res.response.success == 1){
                    dhx_alert("修改成功",function(){
                        window.location.href = "hdw_order.html"
                    });
                }else{
                    dhx_alert(res.response.return_code)
                }
    ...
    $("input:text").bind("input propertychange",function(){
        ...
        $scope.so_name=document.getElementById("parame_c").value;
        $scope.so_mobile=document.getElementById("parame_d").value;
