...
app.controller('myCtrl', function($scope,$http) {
    var get_url = "api/v1.0/admin/aboutus?";//get数据接口
    var del_url  = "api/v1.0/admin/aboutus";//删除接口
    var add_url  ="company_detail_add.html";//点击新增跳转地址
    ...
    function init(){
        if(localStorage.getItem("token") == undefined ||localStorage.getItem("token") == null){
            ...
            myGrid.setHeader("#master_checkbox,序号,标题,添加时间,修改时间,<center>操作</center>");//设置表头
            ...
            myGrid.setInitWidths("50,100,*,*,*,*");//设置表格初始宽度
            ...
            myGridjiazai(1);//加载数据
            myCalendar = new dhtmlXCalendarObject(["web_user_dim_star_add","web_user_dim_end_add","web_user_dim_star_edit","web_user_dim_end_edit"]);//时间插件绑定
            ...
            .success(function(res){
                if(res.response.success == 1){
                    ...
                    for(var i=0; i < get_data.length;i++){
                        ...
                        //var title = decodeURIComponent(get_data[i].content);
                        //
                        //title = title.replace(/(\n)/g, "");
                        ...
                        myGrid.addRow(str,[
                            ...
                            "<button style='width: 60px;height: 25px;background: none;border: none'  id='see_detail'>查看详情</button>"
                        ],i);
    ...
    $("table").on('click','#see_detail',function(){
        window.location.href = "company_detail_detail.html?"+$scope.this_row_id
    });
    //改
